...
const SmallWeather = data => {
      ...
      <View
        ...
        <View style={{marginTop: object.small ? -40 : object.big ? 6 : -30}}>
          ...
          <Text
            ...
            {getHourAndDay(weather.dt_txt).date}
          </Text>
          <Text
            style={{
              fontSize: 13,
            ...
            {getHourAndDay(weather.dt_txt).hour}
          </Text>
        </View>
      </View>
    );
  ...
  const getHourAndDay = text => {
    if (!text) {
      return {
        date: strings.updating,
        hour: strings.updating,
      };
    }
    ...
    let hour = timeSplit[0] + ':' + timeSplit[1];
    let date = moment(new Date(day)).local().format('dddd, DD MM YYYY');
    return {
      date: date,
      hour: hour,
    };
